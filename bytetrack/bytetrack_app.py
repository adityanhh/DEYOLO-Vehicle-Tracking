...
with col_main:
    if uploaded_video is not None:
        ...
        if run_button:
            ...
            # ==================== EKSPOR DATA CSV & ZIP ====================
            csv_log_path = temp_dir / f"bytetrack_log_{Path(uploaded_video.name).stem}.csv"
            csv_summary_path = temp_dir / f"bytetrack_summary_{Path(uploaded_video.name).stem}.csv"
            ...
            if enable_line:
                ...
                df_summary = pd.DataFrame([
                    {'Metrik': 'Total ID ByteTrack', 'Nilai': tracker.get_total_count()}
                ])
                df_summary.to_csv(str(csv_summary_path), index=False)

            metadata_text = f"""DEYOLO ByteTrack Vehicle Tracking & Counting Report
=====================================================
...
Algoritma Tracker     : ByteTrack (BYTETracker)
...
"""
            ...
            with col_d1:
                st.download_button(
                    ...
                    data=zip_bytes,
                    file_name=f"paket_bytetrack_{Path(uploaded_video.name).stem}.zip",
                ...
                if enable_line and csv_log_path.exists():
                    with open(csv_log_path, "rb") as f_csv:
                        st.download_button(
                            ...
                            data=f_csv,
                            file_name=f"bytetrack_log_{Path(uploaded_video.name).stem}.csv",
                            mime="text/csv",
                            use_container_width=True
                        )

            # Statistik & Evaluasi
            st.subheader("📊 Ringkasan Statistik Kendaraan (ByteTrack)")
            s1, s2 = st.columns(2)
            with s1:
