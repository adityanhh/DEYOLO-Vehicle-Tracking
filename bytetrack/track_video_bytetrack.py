"""
Script Tracking Video — ByteTrack SOTA + DEYOLO
================================================
Menjalankan pelacakan kendaraan dengan algoritma ByteTrack (Two-Stage IoU + Kalman Filter)
dan Virtual Counting Line (Tripwire) untuk traffic counting bebas duplikat.

Contoh Penggunaan:
------------------
1. Menggunakan model DEYOLO (Dual-Input Pseudo-IR Sobel):
   python bytetrack/track_video_bytetrack.py --source video_test.mp4 --weights weights/exp05_best.pt --show

2. Menggunakan Counting Line (Garis Hitung pada 60% tinggi layar):
   python bytetrack/track_video_bytetrack.py --source video_test.mp4 --weights weights/exp05_best.pt --line-y 0.60 --show

3. Menyimpan hasil tracking ke file output mp4:
   python bytetrack/track_video_bytetrack.py --source video_test.mp4 --weights weights/exp05_best.pt --output bytetrack_results/hasil_bytetrack.mp4
"""
...
def generate_pseudo_ir(rgb_bgr, method="sobel"):
    """Generate Pseudo-IR dari frame RGB (BGR OpenCV)."""
...
def draw_hud(frame, fps, active_count, count_data, model_name):
    """Menggambar HUD overlay statistik ByteTrack pada frame."""
    h, w = frame.shape[:2]

    # Panel background semi-transparan di pojok kiri atas
    overlay = frame.copy()
    cv2.rectangle(overlay, (15, 15), (360, 165), (20, 20, 20), -1)
    alpha = 0.70
    cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0, frame)

    cv2.rectangle(frame, (15, 15), (360, 165), (100, 100, 100), 1)

    # Info Teks
    cv2.putText(frame, "ByteTrack Vehicle Tracker", (25, 42),
...
def draw_counting_line(frame, line_y):
    """Menggambar garis virtual tripwire horizontal pada frame."""
    h, w = frame.shape[:2]
    # Garis bayangan hitam
    cv2.line(frame, (0, line_y), (w, line_y), (0, 0, 0), 4, cv2.LINE_AA)
    # Garis utama neon cyan
...
def parse_args():
    parser = argparse.ArgumentParser(description="ByteTrack Vehicle Tracking with DEYOLO / YOLOv8")
    ...
    parser.add_argument("--conf", type=float, default=0.20,
                        help="Minimum detection confidence score (default: 0.20)")
    parser.add_argument("--iou", type=float, default=0.70,
                        help="NMS IoU threshold (default: 0.70)")
    parser.add_argument("--track-thresh", type=float, default=0.45,
                        help="ByteTrack High-score threshold (default: 0.45)")
    parser.add_argument("--match-thresh", type=float, default=0.80,
                        help="ByteTrack IoU matching threshold (default: 0.80)")
    parser.add_argument("--track-buffer", type=int, default=30,
                        help="Jumlah frame simpan lost tracks sebelum dihapus (default: 30)")
    parser.add_argument("--line-y", type=float, default=0.55,
                        help="Posisi vertikal Counting Line (rasio 0.0 - 1.0, default: 0.55)")
    parser.add_argument("--output", type=str, default="bytetrack_results/output.mp4",
...
def run_bytetrack():
    ...
    print("=" * 65)
    print("🚗 BYTETRACK SOTA VEHICLE TRACKING PIPELINE")
    print("=" * 65)
    ...
    # Inisialisasi ByteTracker & CountingLine
    tracker = BYTETracker(
    ...
    print("\n🚀 Memulai proses ByteTrack... (Tekan 'q' untuk berhenti, 'space' untuk jeda)")
    ...
    try:
        while cap.isOpened():
            ...
            if args.show:
                cv2.imshow("ByteTrack Vehicle Tracking (DEYOLO)", display_frame)
    ...
    print("\n\n" + "=" * 65)
    print("🎉 PROSES BYTETRACK SELESAI")
    print("=" * 65)
