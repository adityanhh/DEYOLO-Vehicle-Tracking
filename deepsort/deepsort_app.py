...
with col_ctrl:
    ...
    st.caption(f"ℹ️ *{model_cfg.get('desc', '')}*")
...
with col_main:
    if uploaded_video is not None:
        ...
        if run_button:
                ...
                while cap.isOpened():
                    ...
                    if processed_count % 5 == 0 or frame_idx >= total_frames:
                        ...
                        live_preview.image(
                            ...
                            caption=f"Live DeepSORT Preview (Frame {frame_idx}/{total_frames})",
                            width="stretch"
                        )
            ...
            # ==================== EKSPOR DATA CSV & ZIP ====================
            csv_log_path = temp_dir / f"deepsort_log_{Path(uploaded_video.name).stem}.csv"
            csv_summary_path = temp_dir / f"deepsort_summary_{Path(uploaded_video.name).stem}.csv"
            ...
            if enable_line:
                ...
                df_summary = pd.DataFrame([
                    {'Metrik': 'Total ID DeepSORT Terkonfirmasi', 'Nilai': tracker.get_total_count()}
                ])
                df_summary.to_csv(str(csv_summary_path), index=False)

            metadata_text = f"""DEYOLO DeepSORT Vehicle Tracking & Counting Report
=====================================================
Nama File Video       : {uploaded_video.name}
...
Algoritma Tracker     : DeepSORT (Appearance Re-ID + Kalman)
Model Deteksi         : {Path(model_cfg["path"]).name}
...
"""
            ...
            with col_d1:
                st.download_button(
                    ...
                    data=zip_bytes,
                    file_name=f"paket_deepsort_{Path(uploaded_video.name).stem}.zip",
                ...
                if enable_line and csv_log_path.exists():
                    with open(csv_log_path, "rb") as f_csv:
                        st.download_button(
                            ...
                            data=f_csv,
                            file_name=f"deepsort_log_{Path(uploaded_video.name).stem}.csv",
                            mime="text/csv",
                            use_container_width=True
                        )
